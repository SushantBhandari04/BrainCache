...
export function ShareModal({ open, onClose, spaceId, spaceName, onShareChange, isSpaceOwner: propIsSpaceOwner }: ShareModalProps) {
    ...
    async function fetchShareState() {
        ...
        setError(null);
        try {
            const res = await axios.get(`${BACKEND_URL}/api/v1/spaces/${spaceId}/share`, {
    ...
    async function updateSharing(share: boolean) {
        ...
        setError(null);
        try {
            const res = await axios.post(`${BACKEND_URL}/api/v1/spaces/${spaceId}/share`, { share }, {
                headers: { Authorization: localStorage.getItem("token") || "" }
            });
            if (share) {
                setIsShared(true);
                setCurrentHash(res.data.hash);
                onShareChange?.(spaceId, res.data.hash);
            } else {
                setIsShared(false);
                setCurrentHash(null);
                onShareChange?.(spaceId, null);
            }
        } catch (e) {
    ...
    async function nativeShare() {
        ...
        try {
            await navigator.share({
                title: shareTitle,
                text: "Here's a BrainCache space I wanted to share with you.",
                url: shareLink
            });
        } catch (e) {
    ...
    function shareViaEmail() {
        ...
        const body = encodeURIComponent(`Hi,\n\nTake a look at this BrainCache space:\n${shareLink}\n\nThanks!`);
    ...
    return (
        <div className="fixed inset-0 bg-black/35 backdrop-blur-sm flex justify-center items-center z-50 px-3 sm:px-4">
            <div className="bg-white w-full max-w-xl sm:max-w-2xl rounded-2xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
                <div className="flex items-center justify-between px-4 sm:px-6 py-4 sm:py-5 border-b border-gray-100">
                    <div>
                        ...
                        <h2 className="text-lg sm:text-xl md:text-2xl font-semibold text-gray-900 mt-1">
                    ...
                    {spaceId ? (
                        <>
                            <div className="mb-5 sm:mb-6 flex flex-col gap-2">
                                <p className="text-[11px] sm:text-xs uppercase tracking-wide text-gray-500 font-medium">Space</p>
                                <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 sm:gap-3">
                                    <div>
                                        <h3 className="text-base sm:text-lg font-semibold text-gray-900 break-words">{spaceName || "Untitled space"}</h3>
                            ...
                            {activeTab === "comments" ? (
                                <div className="min-h-[320px] sm:min-h-[380px] max-h-[480px] overflow-y-auto flex flex-col">
                                    <SpaceComments 
                                        spaceId={spaceId} 
                                        spaceName={spaceName}
                                ...
                                <>
                                    {loading ? (
                                        ...
                                        <>
                                            ...
                                            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between bg-gray-900 text-white rounded-2xl px-4 sm:px-6 py-4 gap-3 sm:gap-4">
                                                <div>
                                                    ...
                                                    <p className="text-sm sm:text-lg font-semibold">Stop sharing this space</p>
                                                </div>
                                        ...
                                        <div className="bg-gray-50 border border-dashed border-gray-300 rounded-2xl p-5 sm:p-6 text-center">
                                            ...
                                            <h3 className="text-sm sm:text-lg font-semibold text-gray-900 mb-1 sm:mb-2">Space is private</h3>
                                            <p className="text-gray-600 text-xs sm:text-sm mb-4 sm:mb-5">Enable sharing to generate a private link that you can send to collaborators.</p>
                        ...
                        <div className="text-center text-gray-600">
                            <p className="text-sm">Select a space on the dashboard to manage sharing.</p>
                        </div>
                    )}
                </div>
            </div>
